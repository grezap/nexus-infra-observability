...
| 0.I.6 | Fleet-wide shipper rollout | **SEALED 2026-05-28** (source-level: Vector added to `nexus_observability` shared ansible role + `scripts/fleet-vector-rollout.ps1` for live retrofit; ALL 12 currently-running Linux fleet nodes have nexus-vector.service active shipping to Loki HA pair) | smoke-0.I.6 ALL CHECKS GREEN (default mode; end-to-end log flow opt-in via `-EndToEnd`) |
| 0.I.7 | Close-out (canon + tag v0.1.0) | **SEALED 2026-05-28** -- `nexus-infra-observability v0.1.0` tagged; 3-layer canon sweep complete; 39 transients permanently fixed across §3.A-§3.F | — |
...
### §3.F Phase 0.I.6 (Fleet-wide shipper rollout) apply-time transients

| # | Symptom | Root cause | Permanent fix |
|---|---|---|---|
| T34 | First `fleet-vector-rollout.ps1` execution: nexus-vector.service crashloop, `code=exited, status=78/CONFIG` on every node. `vector validate` shows `missing field 'encoding' in 'sinks.otel_logs'`. | Vector's `opentelemetry` sink type in 0.50.0 has a required `encoding` field that I hadn't set. Plus the architectural realization: routing fleet logs through OTel Collector adds a hop for no benefit when Vector can ship directly to Loki (the OTel Collector pair is for app-emitted telemetry, not infra logs). | `packer/_shared/ansible/roles/nexus_observability/tasks/main.yml` + `scripts/fleet-vector-rollout.ps1` -- swap the `opentelemetry` sink for Vector's native `loki` sink: `endpoint: https://loki.nexus.lab:3100` + `encoding: { codec: json }` + static `labels: { fleet: nexusplatform }`. TLS: `verify_certificate: false` (interim until the obs PKI CA bundle distributes fleet-wide). Caught 2026-05-28 in the first rollout pass. |
| T35 | After T34 fix the loki sink loads OK but Vector still errors: `data_dir "/var/lib/vector/" does not exist`. | Vector's default `data_dir` is `/var/lib/vector/` but my ansible role created `/var/lib/nexus-vector/` (the NexusPlatform convention). Vector won't auto-fallback to `$HOME` for the system user. | Added `data_dir: /var/lib/nexus-vector` to vector.yaml (both the ansible role's content + the live-rollout script's heredoc). Vector then uses the NexusPlatform-convention path explicitly. Caught 2026-05-28 in the second rollout pass. |
| T36 | Rollout reports "Failed: 1 -- registry-pg-2" while SSH-probe says reachable. Direct `curl https://packages.timber.io/...` on the failing node errors with `Could not resolve host: packages.timber.io`. | Some fleet nodes have empty/stale `/etc/resolv.conf` (per [[deb13-baseline-dns-resolver]]). The 0.I.6 baseline isn't refreshed at apply-time; defensive write was missing from the rollout script. | `scripts/fleet-vector-rollout.ps1` -- add defensive `nameserver 192.168.70.1` write before `curl`. Live recovery: `ssh nexusadmin@<ip> 'sudo bash -c "echo nameserver 192.168.70.1 > /etc/resolv.conf"'`. Caught 2026-05-28; same root cause as T2 of 0.I.1 / 0.L.2 lessons. |
| T37 | Smoke gate's `hostnameToIp` dict had the wrong IPs for vault-1/2/3 + vault-transit (claimed `.13/.14/.15/.16` instead of the actual `.121/.122/.123/.124`). Rollout script had the same wrong IPs + ALSO wrong registry IPs (claimed `.117/.118/.121/.122` instead of `.115/.116/.117/.118`). Result: `Vector ghost-installed on vault-1/vault-2 by IP collision (registry-pg-1/2 IPs in the rollout list happened to be vault-1/2's actual IPs)`. | Hand-typed fleet inventory drift -- the IP table wasn't cross-checked against `nexus-platform-plan/docs/infra/vms.yaml`. | Both scripts: fixed vault block (`.121-.124`) + registry block (`.115-.118`). Caught 2026-05-28 in smoke run. Lesson: future fleet-inventory scripts MUST derive IPs from `vms.yaml` (single source of truth) rather than hand-coding. Tracked for follow-up: `scripts/lib/load-fleet-inventory.ps1`. |
