/*
 * role-overlay-prom-vault-agents.tf -- Phase 0.I.1, ADR-0038
 *
 * Installs nexus-vault-agent on each of the 2 Prom HA nodes. Port of the
 * iceberg vault-agents overlay; sidecar filename
 * vault-agent-observability-prom-<n>.json + the prom node set.
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars (security env
 * writes them). ERROR (not WARN+skip) if absent.
 *
 * Selective ops: var.enable_prom_vault_agents (master) AND per-host
 *                var.enable_prom_<n>_vault_agent.
 */

locals {
  prom_vault_agent_specs = {
    "prom-1" = { vm_ip = "192.168.70.170", enabled = var.enable_prom_1_vault_agent }
    "prom-2" = { vm_ip = "192.168.70.171", enabled = var.enable_prom_2_vault_agent }
  }

  prom_vault_agent_active = {
    for host, spec in local.prom_vault_agent_specs : host => spec
    if var.enable_prom_vault_agents && spec.enabled
  }

  prom_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_obs_creds_dir, "$HOME", "~"))
  prom_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "prom_vault_agent" {
  for_each = local.prom_vault_agent_active

  triggers = {
    creds_file_path   = "${local.prom_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json"
    creds_file_hash   = filesha256("${local.prom_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json")
    nftables_id       = length(null_resource.prom_nftables_backplane) > 0 ? null_resource.prom_nftables_backplane[0].id : "disabled"
  ...
  depends_on = [null_resource.prom_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.prom_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json'
      $caBundlePath = '${local.prom_va_ca_bundle_expanded}'
      $sshUser      = '${var.obs_node_user}'
      $bootTimeout  = ${var.obs_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[prom-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the obs AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[prom-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[prom-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[prom-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[prom-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[prom-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[prom-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[prom-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[prom-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.I.1 (Prom HA mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.I.1 -- Prom HA mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[prom-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[prom-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[prom-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[prom-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[prom-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
