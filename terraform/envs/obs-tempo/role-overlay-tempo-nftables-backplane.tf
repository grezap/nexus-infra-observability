/*
 * role-overlay-tempo-nftables-backplane.tf -- Phase 0.I.3, ADR-0038
 *
 * Pushes the per-cluster nftables ruleset to all 3 Tempo nodes + `nft -f`.
 * Trust VMnet10 backplane (memberlist gossip on :7946 + gRPC :9095 between
 * Tempo nodes for replication/queries) + open Tempo :3100 push API on
...
locals {
  tempo_all_nodes = {
    "tempo-1" = "192.168.70.175"
    "tempo-2" = "192.168.70.176"
    "tempo-3" = "192.168.70.177"
  }
}

resource "null_resource" "tempo_nftables_backplane" {
  count = var.enable_tempo_nftables_backplane ? 1 : 0

  triggers = {
    node_ips     = join(",", values(local.tempo_all_nodes))
    nftables_v   = "1"
  ...
  depends_on = [
    module.tempo_1, module.tempo_2, module.tempo_3,
  ]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      $nodes       = @{ ${join("; ", [for h, ip in local.tempo_all_nodes : "'${h}' = '${ip}'"])} }
...
table inet filter {
    chain input {
        ...
        iifname "nic0" ip saddr 192.168.70.0/24 tcp dport { 3200, 4317, 4318 } accept comment "Tempo HTTP API :3200 + OTLP gRPC :4317 + OTLP HTTP :4318 from VMnet11 (Grafana, OTel Collector)"
        counter drop
    }
...
'@
      ...
      foreach ($entry in $nodes.GetEnumerator()) {
        $hostName = $entry.Key
        $ip       = $entry.Value
        Write-Host "[tempo-nftables $hostName] waiting for SSH + firstboot marker..."
        ...
        if (-not $booted) { throw "[tempo-nftables $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0 -or $out -notmatch 'NFT_OK') { Write-Host $out.Trim(); throw "[tempo-nftables $hostName] nft -f failed (rc=$LASTEXITCODE)" }
        Write-Host "[tempo-nftables $hostName] ruleset applied"
      }
    PWSH
  }
}
