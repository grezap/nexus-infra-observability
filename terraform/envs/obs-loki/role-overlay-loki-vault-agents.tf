/*
 * role-overlay-loki-vault-agents.tf -- Phase 0.I.2, ADR-0038
 *
 * Installs nexus-vault-agent on each of the 3 Loki nodes. Sidecar filename
 * vault-agent-observability-loki-<n>.json (same shape as the prom sidecars,
 ...
 *
 * Selective ops: var.enable_loki_vault_agents (master) AND per-host
 *                var.enable_loki_<n>_vault_agent.
 */

locals {
  loki_vault_agent_specs = {
    "loki-1" = { vm_ip = "192.168.70.172", enabled = var.enable_loki_1_vault_agent }
    "loki-2" = { vm_ip = "192.168.70.173", enabled = var.enable_loki_2_vault_agent }
    "loki-3" = { vm_ip = "192.168.70.174", enabled = var.enable_loki_3_vault_agent }
  }

  loki_vault_agent_active = {
    for host, spec in local.loki_vault_agent_specs : host => spec
    if var.enable_loki_vault_agents && spec.enabled
  }

  loki_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_obs_creds_dir, "$HOME", "~"))
  loki_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "loki_vault_agent" {
  for_each = local.loki_vault_agent_active

  triggers = {
    creds_file_path   = "${local.loki_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json"
    creds_file_hash   = filesha256("${local.loki_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json")
    nftables_id       = length(null_resource.loki_nftables_backplane) > 0 ? null_resource.loki_nftables_backplane[0].id : "disabled"
  ...
  depends_on = [null_resource.loki_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.loki_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json'
      $caBundlePath = '${local.loki_va_ca_bundle_expanded}'
      $sshUser      = '${var.obs_node_user}'
      $bootTimeout  = ${var.obs_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[loki-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the obs AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[loki-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[loki-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[loki-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[loki-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[loki-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[loki-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[loki-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[loki-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.I.2 (Loki SSD mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.I.2 -- Loki SSD mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[loki-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[loki-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[loki-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[loki-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[loki-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
