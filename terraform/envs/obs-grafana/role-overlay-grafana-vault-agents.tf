/*
 * role-overlay-grafana-vault-agents.tf -- Phase 0.I.4, ADR-0038
 *
 * Installs nexus-vault-agent on each of the 4 Grafana-tier nodes (2 app + 2 PG).
 * Sidecar filename vault-agent-observability-<host>.json (same shape as the
 * prom/loki/tempo sidecars, provisioned together by the security env's
 * obs-approles overlay -- all 14 obs hosts are seeded from day one).
 *
 * Cross-env coupling: reads the AppRole creds JSON sidecars (security env
 * writes them). ERROR (not WARN+skip) if absent.
 *
 * Selective ops: var.enable_grafana_vault_agents (master) AND per-host
 *                var.enable_grafana_<n>_vault_agent.
 */

locals {
  grafana_vault_agent_specs = {
    "grafana-1"    = { vm_ip = "192.168.70.178", enabled = var.enable_grafana_1_vault_agent }
    "grafana-2"    = { vm_ip = "192.168.70.179", enabled = var.enable_grafana_2_vault_agent }
    "grafana-pg-1" = { vm_ip = "192.168.70.180", enabled = var.enable_grafana_pg_1_vault_agent }
    "grafana-pg-2" = { vm_ip = "192.168.70.181", enabled = var.enable_grafana_pg_2_vault_agent }
  }

  grafana_vault_agent_active = {
    for host, spec in local.grafana_vault_agent_specs : host => spec
    if var.enable_grafana_vault_agents && spec.enabled
  }

  grafana_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_obs_creds_dir, "$HOME", "~"))
  grafana_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "grafana_vault_agent" {
  for_each = local.grafana_vault_agent_active

  triggers = {
    creds_file_path      = "${local.grafana_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json"
    creds_file_hash      = filesha256("${local.grafana_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json")
    nftables_id          = length(null_resource.grafana_nftables_backplane) > 0 ? null_resource.grafana_nftables_backplane[0].id : "disabled"
    vault_version        = var.vault_agent_version
    grafana_va_overlay_v = "1"

    destroy_vm_ip    = each.value.vm_ip
    destroy_ssh_user = var.obs_node_user
  }

  depends_on = [null_resource.grafana_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.grafana_va_creds_dir_expanded}/vault-agent-observability-${each.key}.json'
      $caBundlePath = '${local.grafana_va_ca_bundle_expanded}'
      $sshUser      = '${var.obs_node_user}'
      $bootTimeout  = ${var.obs_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[grafana-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the obs AppRole sidecars."
      }
      $creds     = Get-Content $credsFile | ConvertFrom-Json
      $roleId    = $creds.role_id
      $secretId  = $creds.secret_id
      $vaultAddr = $creds.vault_addr
      if (-not $roleId -or -not $secretId) { throw "[grafana-va $hostName] creds JSON missing role_id or secret_id" }
      if (-not (Test-Path $caBundlePath)) { throw "[grafana-va $hostName] CA bundle $caBundlePath missing -- run security env apply first." }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[grafana-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[grafana-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[grafana-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[grafana-va $hostName] installing Vault Agent v$vaultVersion"
      $installScript = @"
set -euo pipefail
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $installOut.Trim(); throw "[grafana-va $hostName] vault binary install failed (rc=$LASTEXITCODE)" }
...
"@
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host $stageOut.Trim(); throw "[grafana-va $hostName] credential staging failed (rc=$LASTEXITCODE)" }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.I.4 (Grafana HA mTLS).
...
"@
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.I.4 -- Grafana HA mTLS)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) { Write-Host $finalOut.Trim(); throw "[grafana-va $hostName] config/service setup failed (rc=$LASTEXITCODE)" }
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[grafana-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      $tokenCheck = (ssh @sshOpts "$sshUser@$vmIp" "sudo test -s /var/run/nexus-vault-agent/token && echo TOKEN_PRESENT" 2>&1 | Out-String).Trim()
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') { throw "[grafana-va $hostName] AppRole login appears to have failed (token sink empty)" }
      Write-Host "[grafana-va $hostName] nexus-vault-agent.service active; AppRole authenticated"
    PWSH
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[grafana-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files"
