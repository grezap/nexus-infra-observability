/*
 * obs-loki-node -- NexusPlatform Loki node template (Phase 0.I.2, ADR-0038).
 *
 * Per-engine template. Installs Loki 3.x as a single-binary in simple-scalable
 * mode + logcli. Go binary; no JVM. Three instances clone into the
 * 01-foundation tier extension per vms.yaml:
 *   - loki-1/2/3 (.172/.173/.174)
 *
 * Each Loki node runs ALL components (read + write + backend). Memberlist
 ...
 * storage is MinIO bucket `loki` (the 0.L.1 object store) via the dedicated
 * `nexus-loki-app` MinIO tenant + scoped `loki-tenant` policy (ADR-0038 +
 * 0.I.2 obs-tenants).
 *
 *   - OS: Debian 13. Default RAM 4 GB (Loki ingest + query + TSDB shipper at lab scale).
 *   - Dual-NIC: client traffic (push :3100 + gRPC :9095) on VMnet11;
 *     memberlist ring (:7946) on VMnet10 backplane.
 *
 * nexus-loki.service is delivered DISABLED. The Terraform obs-loki env's role
 * overlays render /etc/nexus-loki/loki.yaml (memberlist ring members + S3
 * backend with KV-rendered access/secret keys + TLS) + /etc/nexus-loki/web.yml,
 * then enable + start all 3 nodes in parallel so the ring forms cluster-wide.
 *
 * Build:   cd packer/obs-loki-node; packer init .; packer build .
 */
...
source "vmware-iso" "obs-loki-node" {
  vm_name          = var.vm_name
  ...
  vmx_data = {
    "annotation"           = "obs-loki-node template (Phase 0.I.2, ADR-0038) -- built by Packer; Grafana Loki ${var.loki_version} single-binary simple-scalable"
...
build {
  name    = "obs-loki-node"
  sources = ["source.vmware-iso.obs-loki-node"]
  ...
  provisioner "ansible-local" {
    ...
    role_paths = [
      ...
      "ansible/roles/obs_loki",
    ]
    extra_arguments = [
      "--extra-vars", "target_user=${var.ssh_username}",
      "--extra-vars", "obs_loki_version=${var.loki_version}",
      "--extra-vars", "obs_loki_download_url=${var.loki_download_url}",
      "--extra-vars", "obs_logcli_download_url=${var.logcli_download_url}",
    ]
  }

  provisioner "shell" {
    inline = [
      "echo '--- obs-loki-node post-install checks ---'",
      "test -x /opt/loki/loki",
      "test -x /usr/local/bin/loki",
      "test -x /usr/local/bin/logcli",
      "test -d /etc/nexus-loki",
      "test -d /var/lib/nexus-loki",
      "systemctl cat nexus-loki.service > /dev/null",
      ...
      "systemctl is-enabled nexus-loki.service 2>&1 | grep -qE '^(disabled|masked)$' || (echo 'ERROR: nexus-loki.service not disabled at bake' && exit 1)",
      "id loki",
      "echo '--- cleanup ---'",
      "sudo apt-get clean",
